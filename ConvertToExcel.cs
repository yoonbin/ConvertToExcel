...
namespace HIS.MS.IV.RM.CO.UI.Common.UC
{
    public class ConvertToExcel
    {
                                              ...
                                              , bool downExcelYn = false, bool exportYn = true, bool isEncrypt = false, string FileName = "",string pt_no = "",string pt_nm = "",bool autoFilter = false)
        {
            ...
            if (grid is HDataGridEx)
            {
                ...
                dataCount = (hdataGridEx.Items as IList).Count;
            }
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(pt_no))
                {
                ...
                if (string.IsNullOrWhiteSpace(pt_nm))
                {
                    foreach (DataColumn item in dt.Columns)
                    {
                ...
                GetStackTrace(1);
                if (his_prgm_id is string && !string.IsNullOrWhiteSpace(his_prgm_id.ToString()))
            ...
            if (isEncrypt)
            {
                ...
                FileName = string.IsNullOrWhiteSpace(FileName) ? Path.Combine(str1, Path.GetRandomFileName() + str2) : FileName;
            }
            ...
            else if (mergeYn == "Y" && (hdataGridEx.Items as IList).Count > 0)
            {
        ...
        public static DataTable GetDataGridViewAsDataTable(HDataGridEx _DataGridView)
        {
            try
            {
                ...
                {
                    if (col.Header is null)
                        col_header = " ";
                    else
                        col_header = string.IsNullOrWhiteSpace(col.Header.ToString()) ? " " : col.Header.ToString();
                    ...
                    foreach (object obj1 in (IEnumerable)_DataGridView.Items)
                    {
                        ...
                        foreach (DataGridColumn column in (Collection<DataGridColumn>)_DataGridView.Columns)
                        {
                            if (column.Visibility == Visibility.Visible || (bool)column.GetValue(DataGridColumnBehavior.ExportToExcelProperty))
                            {
                                ...
                                else if (!string.IsNullOrWhiteSpace(column.SortMemberPath))
                ...
                {
                    IList list = _DataGridView.Items as IList;
        ...
        {
            //if (!string.IsNullOrWhiteSpace(LastCallStack))
            //    return;
        ...
        public static bool LogInsert(string sheetName, bool isOpenPopup, string MenuCd, int dataCount,string pt_no,string pt_nm)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pt_no) && dataCount > 1)
                str2 = pt_no + " 외 " + (dataCount - 1).ToString() + "명";
            else if(!string.IsNullOrWhiteSpace(pt_no) && dataCount == 1)
                str2 = pt_no;
            
            if (!string.IsNullOrWhiteSpace(pt_nm) && dataCount > 1)
                str3 = pt_nm + " 외 " + (dataCount - 1).ToString() + "명";
            else if (!string.IsNullOrWhiteSpace(pt_nm) && dataCount == 1)
                str3 = pt_nm;
